...
# Replace with env variable or config loading in practice
...
def main_worker(rank, world_size):
    ...
    if rank == 0:
        wandb.init(
            project="DiffusionLMRegexAug_selfies_258_gen",
    ...
    selfies_tokenizer = regexTokenizer(max_len=258)
    model = TransformerNetModel2(
        ...
        training_mode='e2e',
        vocab_size=len(selfies_tokenizer),
        experiment_mode='lm',
        logits_mode=1,
    ...
    if rank == 0:
        ...
        print('Model total parameter number:', pytorch_total_params)
        print('SELFIES tokenizer vocab length:', len(selfies_tokenizer))
    ...
    train_dataset = ChEBIdataset(
        dir='.../DATA/',  # replace with actual dataset path
        smi_tokenizer=selfies_tokenizer,
        split='train',
        replace_desc=False,
        corrupt_prob=0.,
        mask_desc=False,
...
def create_argparser():
    ...
    text_defaults = dict(
        ...
        batch_size=8,
        cache_mode='no',
        checkpoint_path='.../checkpoints_selfies_30cos_258_10W',  # replace with actual checkpoint path
        class_cond=False,
