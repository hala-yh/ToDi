...
class GaussianDiffusion:
    """
    A utility class for training and sampling from diffusion models.

    Ported directly from:
    https://github.com/hojonathanho/diffusion/blob/1e0dceb3b3495bbe19116a5e1b3596cd0706c543/diffusion_tf/diffusion_utils_2.py#L42

    :param betas: A 1D numpy array representing the beta values for each diffusion timestep,
                  decreasing from T (the maximum step) to 1.
    :param model_mean_type: Specifies the type of mean the model predicts.
    :param model_var_type: Specifies the type of variance the model predicts.
    :param loss_type: Specifies the type of loss function used in training.
    :param rescale_timesteps: If True, rescale timesteps to a standardized range (0 to 1000).
    :param model_arch: Specifies the model architecture, e.g., 'conv-unet'.
    :param training_mode: Sets the training mode, such as 'emb' for embedding-based or 'e2e' for end-to-end.
    """

    def __init__(
        self,
        *,
        betas,  # Array of beta values for the diffusion process.
        model_mean_type,  # Mean prediction type, e.g., 'eps', 'x_start', 'v'.
        model_var_type,   # Variance prediction type, e.g., 'fixed_small', 'fixed_large'.
        loss_type,  # Loss function type, e.g., 'L1', 'L2', 'VLB'.
        rescale_timesteps=False,  # Whether to rescale timesteps as input to the model.
        model_arch=None,  # Optional model architecture.
        training_mode='emb',  # Default training mode is embedding-based.
    ):
        ...
        # Convert betas to a high-precision float64 numpy array.
        betas = np.array(betas, dtype=np.float64)
        self.betas = betas

        # Ensure betas is 1D and all values are within (0, 1].
        ...
        # Compute cumulative products of (1 - beta) for each timestep.
        alphas = 1.0 - betas
        ...
        # Precompute square roots and related transforms for efficiency.
        ...
        # Log posterior variance, clipped for numerical stability at the beginning of the chain.
        self.posterior_log_variance_clipped = np.log(
            np.append(self.posterior_variance[1], self.posterior_variance[1:])
        )

        # Posterior mean coefficients for computing q(x_{t-1} | x_t, x_0)
    ...
    def p_mean_variance2(
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            #x_start=pred_xstart, x_t=x, t=t)
                x_start=pred_xstart[0], x_t=x, t=t
            )
        else:
    ...
    def p_mean_variance(
        ...
        model_output = model(x, self._scale_timesteps(t),desc[0],desc[1], desc[2], **model_kwargs)
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            if self.model_arch == 'conv-unet':
                assert model_output.shape == (B, C * 2, *x.shape[2:])
                model_output, model_var_values = th.split(model_output, C, dim=1)
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            raise NotImplementedError(self.model_mean_type)
        # print(f"model_mean type: {type(model_mean)}")
        # print(f"model_log_variance type: {type(model_log_variance)}")
        # print(f"pred_xstart type: {type(pred_xstart)}")
    ...
    def p_sample_loop_progressive(
        ...
        if progress:
            ...
            indices = tqdm(indices)
        if desc is not None:
            print('TextOmics Guiding Generation ......')
            # print(f"Length of desc: {len(desc)}")
            # print(f"desc[2]: {desc[2]}")
            desc = (desc[0].to(img.device),desc[1].to(img.device), desc[2].to(img.device))
        for i in indices:
            t = th.tensor([i] * shape[0], device=device)
            with th.no_grad():
                out = self.p_sample(
    ...
    def training_losses_e2e(self, model, micro, t, model_kwargs=None, noise=None):
        ...
        :param model: Model instance used to compute the loss.
        :param micro: Tuple of input data tensors, including:
                      - input_ids: Original input sequences.
                      - desc_state: Descriptor state.
                      - desc_mask: Descriptor attention mask.
                      - corrupt_ids: Corrupted input sequences.
        :param t: Tensor of current timesteps for the batch.
        :param model_kwargs: Additional arguments to pass into the model (optional).
        :param noise: Optional Gaussian noise used for reconstruction.
        :return: A dictionary containing loss terms.
        """
        input_ids = micro[0]
        desc_state = micro[1]
        desc_mask = micro[2]
        corrupt_ids = micro[3]
        gene_expression = micro[4]
        ...
        x_t = self.q_sample(mix_start, t, noise=noise)
        ...
        if self.loss_type == LossType.E2E_KL:
            ...
            model_output, cos_sim_loss = model(x_t, self._scale_timesteps(t), desc_state, desc_mask, gene_expression)
            ...
            if "vb" in terms:
                ...
                terms["loss"] = terms["mse"] + (decoder_nll + tT_loss) + 0.3 * cos_sim_loss
        else:
